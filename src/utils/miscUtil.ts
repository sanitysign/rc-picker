...
type GetRangePlaceholderProps = {
  placeholderStart?: undefined | string;
  placeholderEnd?: undefined | string;
  index: number;
};

type GetRangePlaceholderSignature = {
  (props: { placeholder?: undefined } & GetRangePlaceholderProps): string;
  <T>(props: { placeholder: T[] } & GetRangePlaceholderProps): T;
};

export const getRangePlaceholder: GetRangePlaceholderSignature = <T>({
  placeholder,
  placeholderStart,
  placeholderEnd,
  index,
}: { placeholder?: undefined | T[] } & GetRangePlaceholderProps) => {
  if (Array.isArray(placeholder)) return placeholder[index];
  const val = index === 0 ? placeholderStart : placeholderEnd;
  return val || '';
};
